...
var _require = __webpack_require__(4),
    ResponseBuilder = _require.ResponseBuilder,
    buildIAMPolicy = _require.buildIAMPolicy;
...
var _require3 = __webpack_require__(21),
    SlackDAO = _require3.SlackDAO,
    HistoryDAO = _require3.HistoryDAO;
...
var buildParam = function buildParam(_ref) {
...
var flattenTail = R.compose(R.tail, R.flatten);
var keyValues = R.map(function (kv) {
  return [R.head(kv), flattenTail(kv)];
});

var buildParams = R.pipe(R.toPairs, keyValues, R.map(buildParam), R.flatten, R.join('&'));

var commas = R.join(',');

module.exports.button = function (event, context, callback) {
  var params = {
    scope: commas(scopes),
    client_id: CLIENT_ID
  };

  var slackButton = paths.button + '?' + buildParams(params);
  var responseBuilder = new ResponseBuilder(callback, { slack_button: slackButton });

  responseBuilder.exec();
};

module.exports.authorize = function () {
  var _ref3 = (0, _asyncToGenerator3.default)( /*#__PURE__*/_regenerator2.default.mark(function _callee(event, context, callback) {
    var authorizationToken, responseBuilder, unauthorized, decoded, token, userId, slackDao, policyDocument;
    return _regenerator2.default.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
          ...
          case 5:
            _context.prev = 5;
            decoded = jwt.verify(authorizationToken, JWT_SECRET);
            token = decoded.token, userId = decoded.userId;
            slackDao = new SlackDAO(token);
            _context.next = 11;
            return !slackDao.isAuthorized();
          ...
          case 13:
            policyDocument = buildIAMPolicy(userId, 'Allow', event.methodArn, { token: token, userId: userId });
            return _context.abrupt('return', callback(null, policyDocument));
...
module.exports.oauth = function () {
  var _ref4 = (0, _asyncToGenerator3.default)( /*#__PURE__*/_regenerator2.default.mark(function _callee2(event, context, callback) {
    var code, slackDao, _ref5, slackToken, userId, historyDao, token, responseBuilder, _responseBuilder;
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            ...
            _context2.prev = 1;
            slackDao = new SlackDAO();
            _context2.next = 5;
            return slackDao.authorize(code);

          case 5:
            _ref5 = _context2.sent;
            slackToken = _ref5.slackToken;
            userId = _ref5.userId;
            historyDao = new HistoryDAO(userId);
            _context2.next = 11;
            return historyDao.createUserIfNotExists();

          case 11:
            token = jwt.sign({ token: slackToken, userId: userId }, JWT_SECRET);
            responseBuilder = new ResponseBuilder(callback, { token: token });
          ...
          case 16:
            ...
            _responseBuilder = new ResponseBuilder(callback);

            _responseBuilder.setStatus(500);
            _responseBuilder.setMessage('Database problem');

            _responseBuilder.exec();
...
module.exports = {"paths":{"button":"https://slack.com/oauth/authorize"},"scopes":["read","post"]};

/***/ })
/******/ ])));
//# sourceMappingURL=slack.auth.js.map